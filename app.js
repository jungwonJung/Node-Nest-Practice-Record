...
app.get('/', function(request,response){  // 요청받지않은 페이지 홈화면 요청받으면 index.ejs 페이지출력
    response.render('index') // views 안에 index.ejs 렌더링 정적 미들웨어
    response.end();
})
...
//     var sql = "SELECT * FROM user ORDER BY id DESC"; // 쿼리문 날려주고 select 문
...
app.get('/delete/:id', function(req,res){
    ...
    db.query(sql,[req.params.id],function(err, results, fields){  // :id 값은 req.params.id 에서 얻는다
        ...
        res.redirect('/page'); //다시 조회창으로 가게  무한삭제가능
    });
});
...
app.get('/page', function(req, res, next){  // /page 로들어오면 /page/1 로 리다이렉트시켜주기
    res.redirect('/page/1')  
});
...
app.get('/page/:page', function(req, res, next){
    ...
    db.query(sql, function(err, results){
        if (err) throw err;
        res.render('page', {results : results, page : page , length:results.length-1, page_num:10, pass:true});
    })
})
